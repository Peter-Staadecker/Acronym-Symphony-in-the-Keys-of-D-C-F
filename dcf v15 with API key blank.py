...
# purchase is ignored.
#
# N.B. Should the data names or structures in financialmodelingprep.com change this program may no longer function
# correctly.
#
# Additional assumptions not listed here may be implicit in the code.
#
...
#  --------------------------------------------------------------------------------------------------------------------
# subroutine to delay between successive calls to json, if needed. Uses resource respectfully.
...
def get_jsonparsed_data(url):
    ...
    Receive the content of ``url``, parse it as JSON and return the object.
            Remember that JSON is a human-readable data file consisting of arrays of name-data pairs.
            Example:
            "employees":[
    ...
    global oldTimeStamp
    delay(oldTimeStamp)  # delay x microseconds to use API respectfully
    myContext = ssl.create_default_context(purpose=ssl.Purpose.SERVER_AUTH)
    response = urlopen(url, context=myContext)
    data = response.read().decode("utf-8")
    oldTimeStamp = datetime.now()  # reset baseline for delay
...
stockListInput = ["MSFT","AMZN","NVDA","META","GOOGL","AVGO","PEP","CO","CMSA","TXN","ADBE","INTC","GILD","AMD","ADP"]
listDescription: str = "test of dcf15"
...
colNames = ["Newest yr history", "Oldest yr history", "EPS newest", "Net Income gr % pa",
...
for stock in stockList:
    ...
    try:
        bigDict: Dict = get_jsonparsed_data(urlIncome)
        # returns dictionary in form [{date1:data,symbol:data,....},{date2:data,symbol:data,....}{....
        ...
        NetIncome[0] = finDictNew['netIncome']
        if NetIncome[0] <= 0:
        ...
        NetIncome[1] = finDictOld['netIncome']
        EPS[1] = finDictOld['epsdiluted']
        if EPS[1] == 0:
        ...
        pdResults.loc[stock,
                      ["Newest yr history", "Oldest yr history", "EPS newest", "Net Income gr % pa",
        ...
        if len(quoteRet) == 0:
            ...
            pdResults.loc[stock, "Warnings if any"] = "No share price found"
            continue
        ...
        if mktCap < minMarketCap:
            ...
            print(Fore.BLACK, end="")
            pdResults.loc[stock, "Warnings if any"] = "Market cap is less than $1 Billion."
        ...
        for col in range(2, 20):  # iterate cols 2 to 19
            ...
            # note the +1 in the iloc endpoint below, because endpoint is excluded
            ...
            if (abs(dfIterate.loc["trial npv", col]) < 0.01) and (col <= 19):  # iteration has come to a successful end
                ...
                print("The min. required EPS growth for breakeven is    ",
                      ...
                      "% pa at a dsct rate of ", "{:4.1f}".format(100 * disctFactor), " %")
